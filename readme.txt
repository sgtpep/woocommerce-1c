...
Если PHP выполняется в режиме FastCGI, а 1С при проверке соединения с сервером просит проверить имя пользователя и пароль, хотя они указаны верно, то необходимо использовать полный адрес обмена (заканчивающийся на `.php`) и в файл .htaccess перед строкой "# BEGIN WordPress" вставить:
`
RewriteEngine On
